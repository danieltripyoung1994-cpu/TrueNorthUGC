...
export default function ProgrammaticSEO() {
  ...
  // Determine if it's a city or niche page from URL path
  ...
  const canonicalPath = isCity ? `/city/${slug}` : `/niche/${slug}`;
  ...
  // Fetch creators filtered by location or niche
  const { data: creators, isLoading } = useCreators(
    isNiche ? { niche: nicheData!.name } : undefined
  );
          ...
          <div className="container mx-auto px-4 relative">
            ...
            >
              <Badge className="mb-4 bg-pink-500/10 text-pink-400 border-pink-500/20 hover:bg-pink-500/20">
                ...
                {isCity ? `${cityData!.name}, ${cityData!.province}` : nicheData!.name}
              </Badge>
          ...
          <div className="container mx-auto px-4">
            <div className="flex items-center gap-2 mb-8">
              ...
              <h2 className="text-2xl font-bold">
                {isCity ? `Top Creators in ${cityData!.name}` : `Top ${nicheData!.name} Creators`}
              </h2>
            </div>
        ...
        <section className="py-12 border-t border-white/10">
          <div className="container mx-auto px-4">
            ...
            <div className="flex flex-wrap gap-2">
              {(isCity ? Object.keys(NICHES) : Object.keys(CITIES)).map((key) => (
                <Link
                  key={key}
                  href={isCity ? `/hire-${key}-ugc-creators` : `/ugc-creators-${key}`}
                  className="text-sm text-muted-foreground hover:text-pink-400 transition-colors underline-offset-2 hover:underline"
                >
                  {isCity ? `Hire ${NICHES[key].name} Creators` : `UGC Creators in ${CITIES[key].name}`}
                </Link>
              ))}
            </div>
          </div>
